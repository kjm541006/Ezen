...
public class CartController {
    ...
    @PostMapping("/order")
    public String addToCart(Model model, @ModelAttribute MemberDTO memberDTO, HttpSession session, @RequestParam("productId[]") Long[] productIds, @RequestParam("quantity[]") int[] quantities) {
        ...
        if (loginId != null) {
            ...
            List<CartEntity> orders = cartService.getAllOrders();
            model.addAttribute("orders", orders);
            for (CartEntity order : orders) {
              int totalPrice = order.getQuantity() * order.getProduct().getPrice();
